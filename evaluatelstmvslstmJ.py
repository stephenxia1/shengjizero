...
def evaluate_mixed_joint(q_path, lstm_path, episodes):
    env = Env1()
    state_dim = int(np.prod(env.observation_space.shape))
    action_dim = int(env.action_space.n)

    lstm = DQNLSTM(state_dim, hidden_dim=128, action_dim=action_dim).to(device)
    lstm.load_state_dict(torch.load(q_path, map_location=device))
    lstm.eval()
    ...
    labels = ['Team 0&2 (LSTM)', 'Team 1&3 (Joint LSTM)']
    means  = [m02, m13]
    errs   = [s02, s13]
    x = np.arange(2)
    ...
    plt.xticks(x_pts,['LSTM','Joint LSTM'])
    plt.ylabel('Points')
    plt.title('Points in Hand vs Points Earned: LSTM vs Joint LSTM')
    ...
    for b,var in zip(bars_e, ['End Pts','End Pts']):
        h=b.get_height(); x0=b.get_x()+b.get_width()/2
        plt.text(x0,h+1,f'{var}={h:.1f}',ha='center',va='bottom')
    plt.legend(); plt.tight_layout(); plt.savefig('lstmjointlstm_points_compare.png'); plt.show()
    ...
    plt.title('First-play Card Rank Frequency by Landlord Team: LSTM vs Joint LSTM')
    plt.legend()
    plt.tight_layout()
...
if __name__=='__main__':
    ...
    evaluate_mixed_joint(args.lstm, args.jointlstm, args.episodes)

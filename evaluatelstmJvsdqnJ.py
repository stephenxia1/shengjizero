...
def evaluate_mixed_joint(path1, path2, episodes=500):
    ...
    joint_lstm.load_state_dict(torch.load(path2, map_location=device))
    joint_lstm.eval()
    ...
    labels = ['Team 0&2 (Joint DQN)', 'Team 1&3 (Joint LSTM)']
    means  = [m02, m13]
    errs   = [s02, s13]
    x = np.arange(2)
    ...
    plt.title('JointDQN vs JointLSTM')
    ...
    plt.xticks(x_pts,['Joint DQN','Joint LSTM'])
    plt.ylabel('Points')
    plt.title('Points in Hand vs Points Earned: Joint DQN vs Joint LSTM')
    ...
    for b,var in zip(bars_e, ['End Pts','End Pts']):
        h=b.get_height(); x0=b.get_x()+b.get_width()/2
        plt.text(x0,h+1,f'{var}={h:.1f}',ha='center',va='bottom')
    plt.legend(); plt.tight_layout(); plt.savefig('mixedjoint_points_compare.png'); plt.show()
    ...
    plt.bar(x_rank + width/2, counts13, width, label='First-card Joint LSTM DQN')
    plt.xticks(x_rank, ranks, rotation=45)
    plt.xlabel('Card Rank')
    plt.ylabel('Frequency')
    plt.title('First-play Card Rank Frequency by Landlord Team: Joint DQN vs Joint LSTM')
    plt.legend()
    plt.tight_layout()

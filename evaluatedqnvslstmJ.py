...
def evaluate_mixed_joint(path1, path2, episodes=500):
    ...
    joint_lstm.load_state_dict(torch.load(path1, map_location=device))
    joint_lstm.eval()
    ...
    for ep in range(1, episodes+1):
        ...
        while not done:
            ...
            inp1 = t_player if player == 0 else torch.zeros_like(t_player)
            inp3 = t_player if player == 2 else torch.zeros_like(t_player)
            with torch.no_grad():
                _, h1, h3 = joint_lstm(inp1, inp3, h1, h3)
            if player in (0,2):
                s0 = torch.from_numpy(env.getObs(0)).float().unsqueeze(0).unsqueeze(1).to(device)
                s2 = torch.from_numpy(env.getObs(2)).float().unsqueeze(0).unsqueeze(1).to(device)
                inp1 = s0 if player==0 else torch.zeros_like(s0)
                inp3 = s2 if player==2 else torch.zeros_like(s2)
                with torch.no_grad():
    ...
    labels = ['Team 0&2 (Joint LSTM)', 'Team 1&3 (DQN)']
    means  = [m02, m13]
    errs   = [s02, s13]
    x = np.arange(2)
    ...
    plt.xticks(x_pts,['Joint LSTM','DQN'])
    plt.ylabel('Points')
    plt.title('Points in Hand vs Points Earned: Joint LSTM vs DQN')
    ...
    for b,var in zip(bars_e, ['End Pts','End Pts']):
        h=b.get_height(); x0=b.get_x()+b.get_width()/2
        plt.text(x0,h+1,f'{var}={h:.1f}',ha='center',va='bottom')
    plt.legend(); plt.tight_layout(); plt.savefig('jointlstmdqn_points_compare.png'); plt.show()
    ...
    plt.figure(figsize=(12,6))
    plt.bar(x_rank - width/2, counts02, width, label='First-card Joint LSTM)')
    plt.bar(x_rank + width/2, counts13, width, label='First-card DQN')
    plt.xticks(x_rank, ranks, rotation=45)
    plt.xlabel('Card Rank')
    plt.ylabel('Frequency')
    plt.title('First-play Card Rank Frequency by Landlord Team: Joint LSTM vs DQN')
    plt.legend()
    plt.tight_layout()
    plt.savefig('jointlstmdqn_first_rank_freq.png')
    plt.show()
...
if __name__=='__main__':
    ...
    evaluate_mixed_joint(args.jointlstm, args.dqn, args.episodes)

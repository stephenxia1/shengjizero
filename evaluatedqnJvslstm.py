...
def evaluate_mixed_joint(path1, path2, episodes):
    ...
    joint_q.load_state_dict(torch.load(path1, map_location=device))
    joint_q.eval()


    lstm = DQNLSTM(state_dim, hidden_dim=128, action_dim=action_dim).to(device)
    lstm.load_state_dict(torch.load(path2, map_location=device))
    lstm.eval()
    ...
    labels = ['Team 0&2 (Joint DQN)', 'Team 1&3 (LSTM)']
    means  = [m02, m13]
    errs   = [s02, s13]
    x = np.arange(2)
    ...
    plt.xticks(x_pts,['Joint DQN','LSTM'])
    plt.ylabel('Points')
    plt.title('Points in Hand vs Points Earned: Joint DQN vs DQN')
    # annotate
    ...
    for b,var in zip(bars_e, ['End Pts','End Pts']):
        h=b.get_height(); x0=b.get_x()+b.get_width()/2
        plt.text(x0,h+1,f'{var}={h:.1f}',ha='center',va='bottom')
    plt.legend(); plt.tight_layout(); plt.savefig('jointdqnlstm_points_compare.png'); plt.show()
    ...
    plt.figure(figsize=(12,6))
    plt.bar(x_rank - width/2, counts02, width, label='First-card Joint DQN')
    plt.bar(x_rank + width/2, counts13, width, label='First-card LSTM')
    plt.xticks(x_rank, ranks, rotation=45)
    plt.xlabel('Card Rank')
    plt.ylabel('Frequency')
    plt.title('First-play Card Rank Frequency by Landlord Team: Joint DQN vs LSTM')
    plt.legend()
    plt.tight_layout()
    plt.savefig('jointdqnlstm_first_rank_freq.png')
    plt.show()
...
if __name__=='__main__':
    ...
    evaluate_mixed_joint(args.jointq, args.lstm, args.episodes)

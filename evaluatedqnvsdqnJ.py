...
def evaluate_mixed_joint(path1, path2, episodes):
    ...
    labels = ['Team 0&2 (Joint DQN)', 'Team 1&3 (DQN)']
    means  = [m02, m13]
    errs   = [s02, s13]
    x = np.arange(2)
    ...
    plt.xticks(x_pts,['Joint DQN','DQN'])
    plt.ylabel('Points')
    ...
    for b,var in zip(bars_e, ['End Pts','End Pts']):
        h=b.get_height(); x0=b.get_x()+b.get_width()/2
        plt.text(x0,h+1,f'{var}={h:.1f}',ha='center',va='bottom')
    plt.legend(); plt.tight_layout(); plt.savefig('jointdqndqn_points_compare.png'); plt.show()
    ...
    plt.title('First-play Card Rank Frequency by Landlord Team: Joint DQN vs DQN')
    plt.legend()
    plt.tight_layout()
    plt.savefig('jointdqndqn_first_rank_freq.png')
    plt.show()
...
if __name__=='__main__':
    ...
    evaluate_mixed_joint(args.jointq, args.dqn, args.episodes)

...
def evaluate_joint_lstm(model_path, episodes=100):
    ...
    print(f"Seats0&2: mean={m0:.2f}, std={s0:.2f}, count={len(team0_returns)}")
    print(f"Seats1&3: mean={m1:.2f}, std={s1:.2f}, count={len(team1_returns)}")

    labels = ['Seats 0 & 2 (Joint LSTM)', 'Seats 1 & 3 (Random)']
    means = [m0, m1]
    errs = [s0, s1]
    ...
    plt.title('Joint LSTM Mean Returns (Playing Against Landlord)')
    plt.tight_layout()
    ...
    plt.xticks(x_pts,['Joint LSTM','Random'])
    plt.ylabel('Points')
    plt.title('Points in Hand vs Points Earned: Joint LSTM vs Random')
    ...
    for b,var in zip(bars_e, ['End Pts','End Pts']):
        h=b.get_height(); x0=b.get_x()+b.get_width()/2
        plt.text(x0,h+1,f'{var}={h:.1f}',ha='center',va='bottom')
    plt.legend(); plt.tight_layout(); plt.savefig('jointlstm_points_compare.png'); plt.show()
    ...
    plt.bar(x - width/2, counts0, width, label='Seats0&2 (Joint LSTM)')
    plt.bar(x + width/2, counts1, width, label='Seats1&3 (Random)')
    plt.xticks(x, ranks, rotation=45)
    plt.ylabel('Frequency')
    plt.title('First-play Card Rank Frequency by Team: Joint LSTM vs Random')
    plt.legend()
    plt.tight_layout()
